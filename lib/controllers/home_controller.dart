...
class HomeController extends ChangeNotifier {
  ...
  /// Ana sayfadaki verileri güncelleme
  Future<void> anaSayfayiGuncelle() async {
    if (_isLoading) return; // Zaten yüklüyorsa çıkış yap
    ...
    try {
      ...
      try {
        ...
        if (kullanici != null) {
          ...
          if (_analizGecmisi.length >= 2) {
            ...
            } catch (kategoriHatasi) {
              debugPrint('Kategori değişimleri hesaplanırken hata: $kategoriHatasi');
              // Hesaplama başarısız olursa boş harita kullan
        ...
        debugPrint('Kullanıcı servisi hatası: $userServiceError');
        // Kullanıcı servisi hatası, uygulama çalışmaya devam etmeli
      }
      ...
      _logger.e('Ana sayfa verileri yüklenirken hata: $e');
      _setError('Veriler yüklenirken bir hata oluştu. Lütfen tekrar deneyin.');
    } finally {
      _setLoading(false);
    }
  }
  
  // Kullanıcı verilerini yükle - Kritik veriler, hemen gerekli
  Future<void> _loadUserData() async {
    try {
      // Burada sadece UI için kritik olan verileri yükle
      final kullanici = await _userService.getCurrentUser();
      if (kullanici != null) {
        _sonAnalizSonucu = kullanici.sonAnalizSonucu;
        _analizGecmisi = kullanici.analizGecmisi;
      ...
      _logger.e('Kullanıcı verileri yüklenirken hata: $e');
    }
  }
  
  // Diğer verileri arka planda asenkron olarak yükle
  Future<void> _loadRestOfDataAsync() async {
    try {
      // Burada UI için anında gerekli olmayan verileri yükle
      // Yoğun işlemleri paralel yürütelim
      await Future.wait([
        _loadKisisellestirilmisTavsiyeler(),
        _loadEkVeriler(),
      ]);
      
      // Tüm yüklemeler tamamlandığında UI'ı güncelle
      notifyListeners();
    } catch (e) {
      _logger.e('Arka plan verileri yüklenirken hata: $e');
      // Hata olsa bile uygulama çalışmaya devam etmeli
    }
  }
  
  // Kişiselleştirilmiş tavsiyeleri yükle
  Future<void> _loadKisisellestirilmisTavsiyeler() async {
    try {
      ...
      _logger.e('Tavsiyeler yüklenirken hata: $e');
    }
  }
  
  // Ek verileri yükle - İsteğe bağlı ve kritik olmayan veriler
  Future<void> _loadEkVeriler() async {
    try {
      // Burada ek verileri yükleyebilirsiniz
      // Örneğin istatistikler, bildirimler vb.
    } catch (e) {
      _logger.e('Ek veriler yüklenirken hata: $e');
    }
  }
